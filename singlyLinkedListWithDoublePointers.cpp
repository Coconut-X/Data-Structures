...
int main() 
{
	...
	while (true) 
	{
		...
		if (choice == 1) 
		{
			...
			{
				expandListArray(lists, capacity);
			}
			lists[listCount] = nullptr;
			currentListIndex = listCount;
			++listCount;
			system("cls");
		...
		else if (choice == 2) 
		{
			system("cls");
			int index;
			cout << "Enter list index (0 to " << listCount - 1 << "): ";
			cin >> index;
		...
		else if (currentListIndex == -1) 
		{
			...
			case 16: 
			{
				system("cls");
				int index;
				cout << "Enter index of list to merge with (0 to " << listCount - 1 << "): ";
				cin >> index;
				if (index >= 0 && index < listCount && index != currentListIndex) {
